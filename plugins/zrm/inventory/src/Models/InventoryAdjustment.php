...
class InventoryAdjustment extends Model
{
    ...
    {
        Log::info('Reversing inventory movements (batch-aware)', [
        ...
        foreach ($this->items as $adjustmentItem) {
            if (! $adjustmentItem->inventoryItem) {
                Log::warning('Skipping item without inventory item', [
            ...
            Log::info('Reversing adjustment item', [
                'adjustment_id' => $this->id,
            ...
            try {
                ...
                if ($quantity < 0 && $adjustmentItem->batchAllocations->isNotEmpty()) {
                    ...
                    Log::info('Using batch allocations for reversal', [
                        'adjustment_item_id' => $adjustmentItem->id,
                ...
                Log::info('Successfully reversed adjustment item', [
                    'adjustment_item_id' => $adjustmentItem->id,
                ]);
            } catch (\Exception $e) {
                Log::error('Failed to reverse adjustment item', [
                    'adjustment_id' => $this->id,
                    'item_id' => $adjustmentItem->id,
                    'error' => $e->getMessage(),
        ...
        Log::info('Inventory movements reversed successfully', [
            'adjustment_id' => $this->id,
