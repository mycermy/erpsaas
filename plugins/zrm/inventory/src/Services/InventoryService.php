...
use Zrm\Inventory\Models\InventoryBatch;
...
class InventoryService
{
    ...
    public function recordMovement(
        ...
        return DB::transaction(function () use (
            ...
            Log::info('recordMovement called', [
                'item_id' => $item->id,
            ...
            if ($referenceType && $referenceId) {
                ...
                Log::info('Checking for existing movement', [
                    'is_reversal' => $isReversal,
                    'notes' => $notes,
                ]);

                $existing = InventoryMovement::withoutGlobalScope(CurrentCompanyScope::class)
                ...
                if ($existing) {
                    Log::info('Found existing movement, returning it', [
                ...
                Log::info('No existing movement found, creating new one');
            }
            ...
            if (($movementType->isOutbound() || ($movementType === MovementType::Adjustment && $quantity < 0)) && $item->track_batches) {
                ...
                if (isset($cogsCalculation['batches']) && ! empty($cogsCalculation['batches'])) {
                    ...
                    if (count($batchAllocations) > 1) {
                        ...
                        return InventoryMovement::withoutGlobalScope(CurrentCompanyScope::class)
                            ->where('inventory_item_id', $item->id)
                            ->where('reference_type', $referenceType)
                            ->where('reference_id', $referenceId)
                            ->where('movement_type', is_object($movementType) && property_exists($movementType, 'value') ? $movementType->value : (string) $movementType)
    ...
    protected function updateStockLevel(
        ...
        $stockLevel = InventoryStockLevel::withoutGlobalScope(CurrentCompanyScope::class)
            ->where('company_id', $item->company_id)
            ->where('inventory_item_id', $item->id)
            ->where('warehouse_id', $warehouse->id)
            ->first();
    ...
    {
        foreach ($batchAllocations as $allocation) {
            $batch = InventoryBatch::withoutGlobalScope(CurrentCompanyScope::class)->find($allocation['batch_id']);
            if ($batch) {
