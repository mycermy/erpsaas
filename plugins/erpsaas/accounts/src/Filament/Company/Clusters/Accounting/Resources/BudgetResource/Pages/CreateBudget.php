...
class CreateBudget extends CreateRecord
{
                    ...
                    \Filament\Schemas\Components\Grid::make(1)
                        ->schema([
                            ...
                            Forms\Components\Select::make('source_budget_id')
                                ...
                                ->searchable()
                                ->required()
                                ->visible(fn(\Filament\Schemas\Components\Utilities\Get $get) => BudgetSourceType::parse($get('source_type'))?->isBudget()),
                            ...
                            Forms\Components\Select::make('source_fiscal_year')
                                ...
                                ->visible(fn(\Filament\Schemas\Components\Utilities\Get $get) => BudgetSourceType::parse($get('source_type'))?->isActuals()),
                        ])->visible(fn(\Filament\Schemas\Components\Utilities\Get $get) => $get('prefill_data') === true),
    ...
    private function getAmountsFromActuals(Account $account, int $fiscalYear, BudgetIntervalType $intervalType): array
    {
        ...
        if (
            $budgetStartDate->month > $budgetEndDate->month ||
    ...
    private function getAmountsFromPreviousBudget(Account $account, int $sourceBudgetId, BudgetIntervalType $intervalType): array
    {
        ...
        if ($intervalType === $sourceBudgetType) {
            ...
            foreach ($previousAllocations as $allocation) {
                ...
                if (
                    $newBudgetStartDate->month > $newBudgetEndDate->month &&
